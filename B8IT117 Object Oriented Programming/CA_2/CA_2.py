    ...
    for index in range(len(dates)):
        ...
        # Continue with the rest dates.
            # if we are still in the same year then
                ...
                accumulator += float(prices[index])
                counter += 1
            # if the year changed, calculate the average for the year
...
def average_price_per_month(dates,prices):
    ...
    for index in range(len(dates)):
        ...
        if year == '':
            ...
            accumulator += float(prices[index])
            counter += 1
            # if the year changed, calculate the average for each month of the year
            # and continue with the next year.
        else:
            average = accumulator / counter
...
#Here we will be calculating the highest and lowest prices yearly
...
def list_of_prices_lowest_to_highest(dates,prices):
    ...
    price_list_ = []
    date_list = dates[:]
    outfile = open('LowToHigh_{:%Y-%m-%d}.txt'.format(today),'w') #Printing the file LowToHigh
    for index in range(len(prices)):
...
def list_of_prices_highest_to_lowest(dates,prices):
    ...
    price_list_ = []
    date_list = dates[:]
    outfile = open('HighToLow_{:%Y-%m-%d}.txt'.format(today), 'w') #Printing the file HighToLow
    for index in range(len(prices)):
        price_list_.append(float(prices[index]))
    for count in range(len(price_list_)):
...
#Main function will be calling all the functions
def main():
    infile = open('GasPrices.txt', 'r')
    infile_list = infile.readlines() # Read the file's contents into a list.
    date_list = []
    price_lists = []
    for line in infile_list: # Display the average price for each year.
        date_list.append(line[0:10])
        price_lists.append(line[11:].rstrip('\n'))
    infile.close()
    average_price_per_year(date_list,price_lists) # Display the average price per year.
    average_price_per_month(date_list,price_lists) # Display the average prices per month.
    highest_lowest_price_per_year(date_list,price_lists) # Display the highest and lowest prices per year.
    list_of_prices_lowest_to_highest(date_list,price_lists) # Display lowest to highest.
    list_of_prices_highest_to_lowest(date_list,price_lists) # Display highest to lowest.
